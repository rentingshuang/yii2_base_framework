window.SINGLE_TAB = "  ";
window.ImgCollapsed = "http://p1.rrkd.cn/group1/M00/18/23/rBBkDViG-CeEakf9AAAAAJPPCAs032.gif";
window.ImgExpanded = "http://p1.rrkd.cn/group1/M00/18/23/rBBkDViG-BOEL6_xAAAAAAYagOE625.gif";
    ...
    if (IsArray(obj)) {
        ...
        if (obj == null) {
            ...
            if (numProps == 0) {
                ...
                isNeedCheck = indent == 1;//等于第一层的时候需要判断下层是否有错
                for (var prop in obj) {
                    var quote = window.QuoteKeys ? "\"" : "";
                    if(isNeedCheck){
                    	var objjs = obj[prop];
                        className = hasError(objjs);
                    }else{
                    	className = hasError(obj);
                    }
                    html += GetRow(indent + 1, "<span class='PropertyName "+className+" '>" + quote + prop + quote + "</span>: " + ProcessObject(obj[prop], indent + 1, ++j < numProps, false, true))
                }
                clpsHtml = window.IsCollapsible ? "</span>" : "";
...
function hasError(obj){
	...
	return (obj.RRKDException || 0)?' red ':'';
}
function FormatLiteral(literal, quote, comma, indent, isArray, style) {
    if (typeof literal == 'string')
        literal = literal.split("<").join("&lt;").split(">").join("&gt;");
    var str = "<span class='" + style + className +"'>" + quote + literal + quote + comma + "</span>";
    if (isArray)
